...
const char No_win_E[] = "Noone has won";
const char No_win_F[] = "Aucun des personnes gagnent";
const char *strings[14][2] = {
														{Names_E,Names_F},
														{Action_E,Action_F},
														{SelCol_E,SelCol_F},
														{P1_Win_E,P1_Win_F},
  ...
  while(notQuit){
    ...
    while(gameisrunning){
					...
					if(placePiece(col, turn) == 0){
						...
						ST7735_DrawString(0,0,(char*) strings[5][language],0xF000);
							...
							if(checkifwon(col, turn) == 0){
									...
									Score1++;
									gameisrunning = false;
							} else if(checkifwon(col, turn) == 2){
									Sound_lose();
									ST7735_FillRect(0,0,160,128,0x000);
									ST7735_FillRect(30,50,100,30,0x00F);
									ST7735_DrawStringS(6,6,(char*)strings[13][language],0xF00F,0xF00F);    
									ST7735_DrawStringS(15,0,"EE319K",0xF00F,0xF00F);
									ST7735_DrawStringS(14,1,(char*) strings[0][language],0xF00F,0xF00F);
									gameisrunning = false;
								}
			...
			if(F0Pressed == 1){
				...
				Wait1ms(20);
				int changedprev2;
				bool playagainchosen2 = false;
				while(!playagainchosen2){
					if (ADC_In() < 2048){
						if(changedprev2 != 0) {
							notQuit = true;
					...
					if(F4Pressed == 1 && changedprev2 == 1){
						Sound_click();
						Wait1ms(20);
						F4Pressed = 0;
						playagainchosen2 = true;
						gameisrunning = true;
						notQuit = true;
						DrawBoard();
						DrawChips();
